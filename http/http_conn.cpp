...
void http_conn::initmysql_result(connection_pool *connPool)
{
    //先从连接池选一个连接
    MYSQL *mysql = NULL;
    connectionRALL(&mysql,connPool);
    //在user表中检索username，passwd数据，浏览器端输入
    if(mysql_query(mysql,"SELECT username,passwd FROM user"))
    {
        LOG_ERROR("SELECT error is:%s\n",mysql_error(mysql));
    }
    //从表中检索完整的结果集
...
void addfd(int epollfd,int sockfd,bool one_shut,int TRIGMode)
{
    epoll_event event;
...
void http_conn::close_http(bool real_close)
{
